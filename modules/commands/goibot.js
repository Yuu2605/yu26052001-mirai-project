...
module.exports.handleEvent = function({ api, event, args, Threads }) {
  ...
  var tl = ["chào bạn tôi là bot của Yu", "bạn kêu tôi có việc gì?", "tôi yêu bạn vai lon", "Yêu em <3", "Hi, chào con vợ bé:3", "Vợ gọi có việc gì không?", "Sử dụng callad để liên lạc với admin!", "Em là bot cute nhất hành tinh", "Nói gì thế con lợn", "Em đây~~~~"];
  ...
  if ((event.body.toLowerCase() == "Cá") ||  (event.body.toLowerCase() == "cá")) {
    return api.sendMessage("đợi tí tao đi lấy cái dép cho ông chủ ❄️", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "bot lon")) {
    return api.sendMessage("Thành viên đã cố ý chửi bot, do đó đã vi phạm luật bot nên bot sẽ out và muốn add lại thì xin liên hệ qua admin qua Fb: Mạnh Tân Vũ để được hỗ trợ\nLink fb nè: https://www.facebook.com/vuvuvi.vi.773", threadID, () =>
      api.removeUserFromGroup(api.getCurrentUserID(), threadID));
  };
  ...
  if ((event.body.toLowerCase() == "@Mạnh Tân Vũ") || (event.body.toLowerCase() == "@Mạnh Tân Vũ")) {
    return api.sendMessage("Ai kêu chủ tao đấy, mà làm ơn đừng tag anh ấy mà hãy liên hệ qua Fb: Mạnh Tân Vũ để được hỗ trợ\nLink fb nè: https://www.facebook.com/vuvuvi.vi.773", threadID);
  };
 ...
 if ((event.body.toLowerCase() == "bsvv nha mng") || (event.body.toLowerCase() == "bsvv nha mng")) {
  ...
  if ((event.body.toLowerCase() == "admin m là ai v bot") || (event.body.toLowerCase() == "admin m la ai v bot")) {
    return api.sendMessage("️Admin tao tên Yu có gì dùng lệnh ❄️admin list hoặc ❄️info để biết thêm chi tiết", threadID);
  };
  ...
  if ((event.body.toLowerCase() == "bot có người yêu chưa") || (event.body.toLowerCase() == "bot co nguoi yeu chua")) {
    return api.sendMessage("Rồi ạ, là anh này này https://www.facebook.com/vuvuvi.vi.773 <3", threadID);
  };
