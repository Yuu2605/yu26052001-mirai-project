module.exports.config = {
	...
	envConfig: {
		YOUTUBE_API: "AIzaSyDMNZtbgUAypH1YpKnLGWrHZN4RPiXFUlA"
	}
	...
	if (/^(http(s)?:\/\/)?((w){3}.)?youtu(be|.be)?(\.com)?\/.+/gm.test(a[0])) try {
		...
		var b = 🎼 Có ${h.length} kết quả trùng với từ khoá tìm kiếm của bạn:\n👇👇👇👇👇\n${u}\nHãy reply(phản hồi) chọn một trong những tìm kiếm trêneturn e.sendMessage({
			attachment: f,
			body: b
		...
		b =🎼 Có ${h.length} kết quả trùng với từ khoá tìm kiếm của bạn:\n👇👇👇👇👇\n${u}\nHãy reply(phản hồi) chọn một trong những tìm kiếm trênreturn e.sendMessage({
			attachment: f,
			body: b
		}, t.threadID, ((e, a) => global.client.handleReply.push({
			name: this.config.name,
