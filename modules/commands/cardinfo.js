...
module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "D-Jukie",
  description: "Táº¡o card thĂ´ng tin ngÆ°á»i dĂ¹ng facebook",
...
module.exports.run = async function ({ api, event, args, Users }) {
  if ((this.config.credits) != "D-Jukie") { return api.sendMessage(`â¡ï¸PhĂ¡t hiá»‡n credits Ä‘Ă£ bá»‹ thay Ä‘á»•i`, event.threadID, event.messageID)}
...
/*---------------------------------------------*/
  ...
  ctx.drawImage(baseAvata, 80, 73, 285, 285);
    var gender = res.data.gender == 'male' ? "Nam" : res.data.gender == 'female' ? "Nù›̀ƒ" : "KhĂ´ng cĂ´ng khai";
  ...
  ctx.textAlign = "start";
  fontSize = 20;
  ctx.fillText(`Â» Name: ${res.data.fullname}`, 455, 172);
  ctx.fillText(`Â» Sex: ${gender}`, 455, 208);
  ctx.fillText(`Â» Follow: ${res.data.follow_user}`, 455, 244);
  ...
  ctx.textAlign = "start";
  fontSize = 20;  
  ctx.fillText(`Â» Profile: ${res.data.url_profile}`, 19, 468);
  ctx.beginPath();
